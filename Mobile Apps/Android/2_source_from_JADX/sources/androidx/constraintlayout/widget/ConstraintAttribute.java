...
public class ConstraintAttribute {
    ...
    public int noOfInterpValues() {
        int i = C01301.f47x66adad53[this.mType.ordinal()];
        if (i == 1 || i == 2) {
            return 4;
        }
        return 1;
    ...
    public float getValueToInterpolate() {
        switch (C01301.f47x66adad53[this.mType.ordinal()]) {
            case 1:
            case 2:
    ...
    public static void setAttributes(View view, HashMap<String, ConstraintAttribute> map) {
        ...
        for (String name : map.keySet()) {
            ...
            try {
                switch (C01301.f47x66adad53[constraintAttribute.mType.ordinal()]) {
                    case 1:
    ...
    public static void parse(Context context, XmlPullParser parser, HashMap<String, ConstraintAttribute> custom) {
        TypedArray a = context.obtainStyledAttributes(Xml.asAttributeSet(parser), C0132R.styleable.CustomAttribute);
        String name = null;
        Object value = null;
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (attr == C0132R.styleable.CustomAttribute_attributeName) {
                name = a.getString(attr);
                if (name != null && name.length() > 0) {
                    name = Character.toUpperCase(name.charAt(0)) + name.substring(1);
                }
            } else if (attr == C0132R.styleable.CustomAttribute_customBoolean) {
                value = Boolean.valueOf(a.getBoolean(attr, false));
                type = AttributeType.BOOLEAN_TYPE;
            } else if (attr == C0132R.styleable.CustomAttribute_customColorValue) {
                type = AttributeType.COLOR_TYPE;
                value = Integer.valueOf(a.getColor(attr, 0));
            } else if (attr == C0132R.styleable.CustomAttribute_customColorDrawableValue) {
                type = AttributeType.COLOR_DRAWABLE_TYPE;
                value = Integer.valueOf(a.getColor(attr, 0));
            } else if (attr == C0132R.styleable.CustomAttribute_customPixelDimension) {
            ...
            } else if (attr == C0132R.styleable.CustomAttribute_customDimension) {
                type = AttributeType.DIMENSION_TYPE;
                value = Float.valueOf(a.getDimension(attr, 0.0f));
            } else if (attr == C0132R.styleable.CustomAttribute_customFloatValue) {
                type = AttributeType.FLOAT_TYPE;
                value = Float.valueOf(a.getFloat(attr, Float.NaN));
            } else if (attr == C0132R.styleable.CustomAttribute_customIntegerValue) {
                type = AttributeType.INT_TYPE;
                value = Integer.valueOf(a.getInteger(attr, -1));
            } else if (attr == C0132R.styleable.CustomAttribute_customStringValue) {
