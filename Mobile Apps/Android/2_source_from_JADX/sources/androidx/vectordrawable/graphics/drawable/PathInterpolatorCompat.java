...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (numPoints > 0) {
            this.f58mX = new float[numPoints];
            this.f59mY = new float[numPoints];
            float[] position = new float[2];
            for (int i = 0; i < numPoints; i++) {
                pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, (float[]) null);
                this.f58mX[i] = position[0];
                this.f59mY[i] = position[1];
            }
            if (((double) Math.abs(this.f58mX[0])) > 1.0E-5d || ((double) Math.abs(this.f59mY[0])) > 1.0E-5d || ((double) Math.abs(this.f58mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f59mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
                throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f58mX[0] + "," + this.f59mY[0] + " end:" + this.f58mX[numPoints - 1] + "," + this.f59mY[numPoints - 1]);
            }
            float prevX = 0.0f;
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f58mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f58mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
